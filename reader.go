...
func decodeLineOfMediaPlaylist(p *MediaPlaylist, wv *WV, state *decodingState, line string, strict bool) error {
		...
		if state.tagMarker {
			for _, marker := range state.markers {
				p.Segments[p.last()].Markers = append(p.Segments[p.last()].Markers, &Marker{
					marker.ID, marker.MarkerType, marker.Duration, marker.Count, marker.Index, marker.Data,
				})
			}
			state.markers = nil
	...
	case strings.HasPrefix(line, "#EXT-X-MARKER:"):
		...
		for k, v := range decodeParamsLine(line[14:]) {
			...
			case "INDEX":
				marker.Index, _ = strconv.ParseInt(v, 10, 64)
			case "DATA":
				marker.Data = v
			}
		}
	...
	case state.tagSCTE35 && state.scte.Syntax == SCTE35_OATCLS && strings.HasPrefix(line, "#EXT-X-CUE-OUT:"):
		// EXT-OATCLS-SCTE35 contains the SCTE35 tag, EXT-X-CUE-OUT contains duration
		duration, err := strconv.ParseFloat(line[15:], 64)
		if err == nil {
			state.scte.Time = duration
		} else {
			params := decodeParamsLine(line[15:])
			if duration, ok := params["DURATION"]; ok {
				state.scte.Time, _ = strconv.ParseFloat(duration, 64)
			}
		}
	...
	case !state.tagSCTE35 && strings.HasPrefix(line, "#EXT-X-CUE-OUT-CONT:"):
		...
		if strings.Contains(line, "SCTE35=") {
