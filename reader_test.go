...
func TestMediaPlaylistWithAdobeTag(t *testing.T) {
	...
	t.Run("media-playlist-with-cue-out-partner-triplelift.m3u8", func(t *testing.T) {
		f, err := os.Open("sample-playlists/media-playlist-with-cue-out-partner-triplelift.m3u8")
		if err != nil {
			t.Fatal(err)
		}
		p, _, err := DecodeFrom(bufio.NewReader(f), true)
	...
	t.Run("media-playlist-with-cue-out-partner-gumgum.m3u8", func(t *testing.T) {
		f, err := os.Open("sample-playlists/media-playlist-with-cue-out-partner-gumgum.m3u8")
		if err != nil {
			t.Fatal(err)
		}
		p, _, err := DecodeFrom(bufio.NewReader(f), true)
